...
int main(int argc, char** argv) {
    ...
    if(argc < 4 || argc > 5) {
        ...
        << "<input file> <number of bytes> [<shared memory size>]" << std::endl;
        return 1;
    }

    if (argc == 5) {
        shared_size = atoi(argv[4]);
    }
    ...
    for (long int i = 0; i < size; ++i) {
        ...
        SYMBOL_TYPE raw_symbol = get_le_uint<SYMBOL_TYPE>(inb); // little endian
        buffer[i] = (raw_symbol <= 1024) ? raw_symbol : 0;
    }
    TIMER_STOP;
    inf.close();
    ...
    std::unique_ptr<uint8_t[]> gap_array
        = std::make_unique<uint8_t[]>(SDIV(enc_table->compressed_size, SUBSEQ_SIZE));
    
    // compress
    TIMER_START(timings, "encoding")
        llhuff::LLHuffmanEncoder::encode_memory(compressed.get(),
                                                enc_table->compressed_size, buffer.data(), gap_array.get(), SUBSEQ_SIZE, size, enc_table);
    TIMER_STOP
    ...
    std::cout << "CR: "
              << (double) (size * sizeof(SYMBOL_TYPE)) / (double) ((enc_table->compressed_size * sizeof(UNIT_TYPE)) + (SDIV(enc_table->compressed_size, SUBSEQ_SIZE)  * sizeof(uint8_t)))
              << std::endl;
    ...
    auto gpu_gap_buf = std::make_shared<cuhd::CUHDGPUMemoryBuffer<uint8_t>>(gap_array.get(), SDIV(enc_table->compressed_size, SUBSEQ_SIZE));
    ...
    TIMER_START(timings, "GPU memcpy HtD")
        ...
        gpu_gap_buf->cpy_host_to_device();
    TIMER_STOP
    ...
    // decode
        cuhd::CUHDGPUDecoder::decode(
         ...
         gpu_gap_buf, SDIV(enc_table->compressed_size, SUBSEQ_SIZE),
    ...
    double p1Avg = 0, p2Avg = 0, p3Avg = 0, p4Avg = 0, tuneAvg = 0;
    ...
    double total = p1Avg + p2Avg + p3Avg + tuneAvg + p4Avg;
    
    double thruGbs = ((double)(size * sizeof(SYMBOL_TYPE))
                      / (total * 1e-6)) / (1024.0 * 1024.0 * 1024.0);
    std::cout << "throughput: " << thruGbs
              << " GB/s" << std::endl;
