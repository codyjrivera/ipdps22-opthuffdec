...
__global__ void print_array(std::uint32_t* array, std::uint32_t size) {
...
__global__ void phase1_decode_subseq(
    ...
    // System-dependent
    __shared__ bool warp_synchronized[16];
    
    const std::uint32_t gid = blockDim.x * blockIdx.x + threadIdx.x;
    std::uint32_t subseq_count = 0;
    ...
    if(gid < total_num_subsequences) {
        ...
        *((uint4*) subsequence) = ((uint4*) in_ptr)[in_pos / 4];
        ...
        while(subsequences_processed < blockDim.x) {
            ...
            // System-dependent
            warp_synchronized[threadIdx.x / warpSize]
                = __all_sync(UINT_MAX, synchronized_flag || current_subsequence_in_block >= blockDim.x);
            ...
            bool block_synchronized = true;
            for (int i = 0; i < blockDim.x / warpSize; ++i) {
                
                block_synchronized = block_synchronized && warp_synchronized[i];
            }
            if (block_synchronized) {
                 break;
            }
        }
        s1[gid] = subseq_count;
...
__global__ void phase4_decode_write_output(
    ...
    while (subsequence_base < blockDim.x) {
        ...
        for (std::uint32_t i = write_start_pos + threadIdx.x;
            ...
            //if (i - write_start_pos >= shared_size) {
            //    printf("oh shoot %d %d tid %d bid %d sid %d oid %d\n", i - write_start_pos, shared_size, threadIdx.x, sequence_index, start_index, blockIdx.x);
            //}
...
void cuhd::CUHDGPUDecoder::decode(
    ...
    if (invocation_count == 0) {
        ...
        std::cout << "above thresh: " << (double) g_thresh/(double) num_subseq << " max: " << max << std::endl;
        CUERR;
    }
