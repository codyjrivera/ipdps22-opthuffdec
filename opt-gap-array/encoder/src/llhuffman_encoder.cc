...
std::shared_ptr<std::vector<llhuff::LLHuffmanEncoder::Symbol>>
    ...
    if(log2(freq.size()) > MAX_CODEWORD_LENGTH) {
        std::cout << log2(freq.size()) << std::endl;
        //return nullptr;
    }
...
void llhuff::LLHuffmanEncoder::encode_memory(UNIT_TYPE* out, size_t size_out,
                                             SYMBOL_TYPE* in, uint8_t* gap_array, size_t subseq_size, size_t size_in,
    ...
    for(i = 0; i < size_out && in_unit < size_in; ++i) {
        if (i % subseq_size == 0) { gap_array[i / subseq_size] = at;
            /*std::cout << "gap array" << i << " " << at << std::endl;*/ }
        ...
        out_ptr[i] = window;
        //std::cout << i << ": " << std::bitset<8 * sizeof(UNIT_TYPE)>(window) << std::endl;
...
std::shared_ptr<cuhd::CUHDCodetable>
    ...
    for(auto &i: enc_table->dict) {    
        ...
        //std::cout << (int) symbol << ": " << length << ": " << std::bitset<8 * sizeof(UNIT_TYPE)>(codeword) << std::endl;
